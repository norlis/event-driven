...
func TestNewInterface(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			typ:         reflect.TypeOf(SampleStruct{}), // Asumiendo que Name no es 'omitempty'
			data:        []byte(`{"value":123}`),
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			val, err := NewInterface(tt.typ, tt.data)

			if (err != nil) != tt.expectErr {
				t.Fatalf("NewInterface() error = %v, expectErr %v", err, tt.expectErr)
			}
			...
			if !tt.expectErr {
				...
				if !reflect.DeepEqual(val, tt.expectedVal) {
					...
					t.Errorf("NewInterface() got = %s (%T), want %s (%T)", actualJSON, val, expectedJSON, tt.expectedVal)
				}
			}
		})
	}
